  ...
  return (
    <Surface>
      <SectionTitle
        ...
        description="Daily task volume for the last 14 days. Hover a day to see who worked on it."
        action={
          data.projectCount > 0 ? (
            <div className="flex items-center gap-2 text-[12px]">
              <span className="rounded-md bg-[#ECFDF3] px-2 py-1 font-semibold tabular-nums text-[#027A48]">
                {dayTotals.completed} done
              </span>
        ...
        <div className="space-y-4">
          <div className="flex min-h-[220px] items-end gap-1.5 overflow-x-auto rounded-xl border border-border bg-muted/30 px-3 py-4 sm:gap-2">
            {data.days.map((day, index) => {
              ...
              const doneH = day.total === 0 ? 0 : Math.round((day.completed / day.total) * stackH);
                ...
                >
                  <div className="flex h-[150px] w-full items-end justify-center">
                    <div
                      ...
                      style={{
                        height: ready ? stackH : 0,
                        transition: `height 650ms cubic-bezier(0.22,1,0.36,1) ${90 + index * 28}ms`,
                      }}
                    >
                      <div
                        className="w-full bg-[#12B76A]"
                        style={{
                          height: ready ? doneH : 0,
                          transition: `height 650ms ease ${110 + index * 28}ms`,
                        }}
                      />
                      <div
                        className="w-full bg-[#F79009]"
                        style={{
                          height: ready ? openH : 0,
                          transition: `height 650ms ease ${130 + index * 28}ms`,
                        }}
                      />
                    </div>
                  </div>
